...
# Fallback proxy — used when you DON'T pass ?proxy= in the URL
# Format: http://user:pass@host:port
FALLBACK_PROXY = "http://brd-customer-hl_54dda161-zone-isp_proxy1:sxf92a7e5g32@brd.superproxy.io:33335"
...
def parse_proxy(proxy_str):
    ...
    # Already a full URL
    if proxy_str.startswith('http://') or proxy_str.startswith('https://') or proxy_str.startswith('socks4://') or proxy_str.startswith('socks5://'):
        return proxy_str
...
async def fetch_products(domain, proxy_str=None, max_retries=5):
    ...
    for attempt in range(max_retries):
        proxy = parse_proxy(proxy_str) if proxy_str else FALLBACK_PROXY
    ...
    return False, "<b>Fetch failed. Use ?variant=ID</b>"
...
admin_env = os.getenv("ADMIN_ID", "[5237309941]")
try:
