...
def _get_cached(domain):
    ...
    ts, result, ttl = entry
    if time.time() - ts > ttl:
        _product_cache.pop(domain, None)
        return None
    return result

def _set_cache(domain, result, ttl):
    _product_cache[domain] = (time.time(), result, ttl)
...
def _pick_cheapest(data, max_price):
    ...
    for p in products:
        for v in p.get("variants", []):
            ...
            try:
                ...
                continue
            if 0 < price <= max_price and price < best_price:
                best_price = price
                best = p
                best_variant = v
    if not best:
        return None
    # Return raw product dict + helper fields, preserves original shape
    result = dict(best)
    result["ok"] = True
    result["chosen_variant"] = best_variant
    result["chosen_price"] = best_price
    return result
...
async def fetch_cheapest_product(site, proxy=None, max_price=8.00, log=print):
    from urllib.parse import urlparse
    domain = urlparse(site).netloc or site

    cached = _get_cached(domain)
    if cached is not None:
        log(f"    [FETCH] [CACHE HIT] Using cached product result for {site}")
        return cached

    url = [
    f"{site.rstrip('/')}/products.json?limit=250",
    f"{site.rstrip('/')}/collections/all/products.json?limit=250",
    f"{site.rstrip('/')}/collections/all.json?limit=250",
]
    log(f"    [FETCH] Checking {url}")

    skip_proxy = _domain_429_strikes.get(domain, 0) >= 2
    if skip_proxy:
        log(f"    [FETCH] Skipping proxy for {domain} (repeated 429s)")

    last_err = None
    for attempt in range(1, 4):  # 3 tries with backoff
        try:
            timeout = aiohttp.ClientTimeout(total=25)
            async with aiohttp.ClientSession(timeout=timeout) as session:
                use_proxy = proxy if (proxy and not skip_proxy) else None
                async with session.get(url, headers=_headers(), proxy=use_proxy) as resp:
                    status = resp.status
                    log(f"    [FETCH] {'Proxy' if use_proxy else 'Direct'} Response Status: {status} (attempt {attempt})")

                    if status == 200:
                        data = await resp.json(content_type=None)
                        _domain_429_strikes[domain] = 0
                        result = _pick_cheapest(data, max_price)
                        if result:
                            _set_cache(domain, result, SUCCESS_TTL)
                            return result
                        err = {"ok": False, "error": "NO_PRODUCT: no product under max price"}
                        _set_cache(domain, err, TRANSIENT_TTL)
                        return err

                    if status == 429:
                        _domain_429_strikes[domain] = _domain_429_strikes.get(domain, 0) + 1
                        last_err = {"ok": False, "error": "Proxy fetch returned status 429"}
                        # try direct fallback once on first attempt
                        if use_proxy and attempt == 1:
                            log(f"    [FETCH] 429 via proxy. Trying direct fallback...")
                            try:
                                async with session.get(url, headers=_headers(), proxy=None) as r2:
                                    log(f"    [FETCH] Direct Response Status: {r2.status}")
                                    if r2.status == 200:
                                        ...
                                        _domain_429_strikes[domain] += 1
                            except Exception as e:
                                log(f"    [FETCH] Direct fallback error: {e}")

                        # exp backoff + jitter before next retry
                        if attempt < 3:
                            sleep_for = random.uniform(2, 8) * attempt
                            log(f"    [FETCH] 429 hit. Backing off {sleep_for:.1f}s before retry {attempt+1}/3")
                            await asyncio.sleep(sleep_for)
                            continue
                        else:
                            log(f"    [FETCH] Failed: Proxy fetch returned status 429")
                            _set_cache(domain, last_err, RATE_LIMIT_TTL)
                            return last_err

                    # other bad statuses
                    last_err = {"ok": False, "error": f"HTTP {status}"}
                    if attempt < 3:
                        await asyncio.sleep(random.uniform(1, 3) * attempt)
                        continue
                    _set_cache(domain, last_err, TRANSIENT_TTL)
                    return last_err

        except asyncio.TimeoutError:
            last_err = {"ok": False, "error": "Timeout fetching products"}
            log(f"    [FETCH] Timeout on attempt {attempt}")
            if attempt < 3:
                await asyncio.sleep(random.uniform(1, 3) * attempt)
                continue
        except Exception as e:
            last_err = {"ok": False, "error": f"Fetch exception: {e}"}
            log(f"    [FETCH] Exception on attempt {attempt}: {e}")
            if attempt < 3:
                await asyncio.sleep(random.uniform(1, 3) * attempt)
                continue

    _set_cache(domain, last_err or {"ok": False, "error": "unknown"}, TRANSIENT_TTL)
