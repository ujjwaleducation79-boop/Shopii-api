# Maxx Telegram Bot - Shopify checker using gg.py
...
def _check_site_fast_sync(site_url, proxy_url=None):
    ...
    try:
        ...
        print(f"[DEBUG] Sync Error for {site_url}: {e}")
        return False, f"Sync Error: {e}"


async def check_site_fast(site_url, proxy_url=None, max_price=50.0, min_price=1.0):
    """DEBUG VERSION - Very lenient"""
    print(f"[DEBUG] Checking site: {site_url}")   # ← Debug print
    
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=50)) as session:
            params = {"site": site_url.strip(), "cc": TEST_CCS[0]}
            if proxy_url:
                params["proxy"] = proxy_url

            print(f"[DEBUG] Calling API: {Maxx_API_URL}/shopify")  # Debug

            async with session.get(f"{Maxx_API_URL}/shopify", params=params, ssl=False) as resp:
                print(f"[DEBUG] API Response Status: {resp.status}")  # Debug
                
                if resp.status != 200:
                    return False, f"API HTTP {resp.status}"

                data = await resp.json()
                print(f"[DEBUG] API Response Data: {data}")  # ← Most Important Debug
                ...
                # Very lenient check
                if status in ["declined", "approved", "charged", "success"] or "declined" in message:
                    print(f"[DEBUG] ✅ Site Accepted: {site_url} | Status: {status}")
                    return True, f"Working ({status.title()})"
                else:
                    print(f"[DEBUG] ❌ Site Rejected: {site_url} | Status: {status} | Msg: {message}")
                    return False, f"Status: {status} | {message[:80]}"

    except Exception as e:
        print(f"[DEBUG] Exception on {site_url}: {e}")
...
def cmd_sh(message):
    ...
    try:
        ...
        err_text = str(e)[:300]
        if DEBUG:
            print(f"[Maxx] /sh exception -> {err_text}")
